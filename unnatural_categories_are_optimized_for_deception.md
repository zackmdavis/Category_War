...
$$P(\mathrm{category} = \mathrm{blegg} | \mathrm{eggness} = 7) =$$

$$\frac{\frac{1}{4} \cdot \frac{12}{25}}{\frac{1}{4} \cdot \frac{12}{25} + 0 \cdot \frac{12}{25} + \frac{1}{8} \cdot \frac{1}{25}} = \frac{24}{25} = 0.96$$
...
To formalize this, let's measure how "good" a category is using the [expected squared error](https://en.wikipedia.org/wiki/Mean_squared_error). "Error" is how much a prediction is wrong by: if you guessed $x$ was 2, but it was actually 5, your error would be $5 - 2 = 3$, and your squared error would be the square of that, $3^2 = 9$. The expected squared error of a probability distribution is, on average, the square of how much your guesses about a sample from that distribution will be wrong. [(The squared error has nicer mathematical properties than the absolute error.)](https://www.benkuhn.net/squared/)

[TODO: fix bug in code and redo calculations and rewrite exposition!!]

For our example of $x$ sampled from $X$ uniformly distributed on $\{1, 2, 3, 4, 5, 6, 7, 8\}$, the initial expected squared error is 10.5. If you learn whether $x$ is even or odd, the expected squared error of your prediction about $x$ only goes down to 10, whereas if you learn whether $x$ is 1–4 or 5–8, your expected squared error plummets to 2.5. By being compact, the "1–4 or 5–8" category system is much more useful for getting _close_ to the right answer than the "even/odd" category system.
...
Then our expected squared error before being told anything about an object is about 27.26. On being told that an object is a blegg (with respect to what that means in our nice factorized naïve Bayes model), our expected squared error plummets to about 2.76. But on being told that an object is a blegg\* (that is, that it's within the squiggly boundary in the second blueness–eggness diagram), our expected squared error only goes down to about 8.24.[^script]

[^script]: [The source code of the Python script that was used to calculate this is available.](TODO: Gist link)
...
[TODO: rewrite to be about humor, not attractiveness]
